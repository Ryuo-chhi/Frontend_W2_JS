...
function getCartTotalAmount() {
  ...
  SHOPPING_CART.forEach((ele) => {
    const p = PRODUCTS.find((product) => ele.id === product.id);
    if (p) {
      result += p.price * ele.quantity;
    }
  });
  return result;
}
...
function removeProductFromCart(productId) {
  // Write your code here
  let removeItem = SHOPPING_CART.find((item) => item.id === productId);
  if (!removeItem) {
    return;
  } else if (removeItem.quantity >= 2) {
    removeItem.quantity--;
  } else {
    let index = SHOPPING_CART.findIndex((item) => item === removeItem);
